window.addEventListener('load', function() {
    ...
    // 获取图片 和 小点
    var imgs = this.document.querySelectorAll('.image');
    ...
    var current = this.document.querySelector('.arr');
    ...
    leftArrow.addEventListener('click', function() {
        if (currentIndex > 0) {
            imgs[currentIndex].classList.remove('arr');
            ...
            dots[currentIndex].classList.remove('square');
            currentIndex = 4;
            imgs[currentIndex].classList.add('arr');
            dots[currentIndex].classList.add('square');
        }
    });
